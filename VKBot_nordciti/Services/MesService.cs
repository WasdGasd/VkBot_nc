    ...
    public class MesService : IMessageService
    {
        ...
        public async Task ProcessMessageAsync(VkMessage message)
        {
            try
            {
                ...
                // (ПЕРЕМЕЩЕНО ВВЕРХ - ДО ОСНОВНОГО SWITCH-CASE)
                // ======================================================
                ...
                // Режим работы - ОБРАТИТЕ ВНИМАНИЕ: убрали обработку "⏰" здесь
                // чтобы не конфликтовать с выбором сеанса
                if (text.ToLower().Contains("режим работы") || text.ToLower().Contains("время работы"))
                {
